...
def main(directory, source, analyses):
  ...
  dump_file = gzip.open(BINARY + '_fulltrace.gz', 'r')
  br_branch_flag = 0
  swi_branch_flag = 0
  mem_flag = 0
  ...
  if opcode_ana_flag == 1:
    ...
    # print '======Compute:Memory:Branch:%d:%d:%d' %(compute_op, mem_op,
    # branch_op)
  ...
  if staticinst_ana_flag == 1:
    ...
    for w in sorted(static_inst.iteritems(), key=operator.itemgetter(1),
                     reverse=True):
      key = w[0]
      value = str(w[1])
  ...
  if mem_ana_flag == 1:
    ...
    for w in sorted(addr_counter.iteritems(), key=operator.itemgetter(1),
                     reverse=True):
      key = w[0]
      value = int(w[1])
  ...
  if branch_ana_flag == 1:
    ...
    for key, value in branch_stream.iteritems():
      for i in range(len(value) - 15):
        ...
        pattern_count += 1
        if curr_pattern in branch_pattern:
          branch_pattern[curr_pattern] += 1
        else:
          branch_pattern[curr_pattern] = 1

    for w in sorted(branch_pattern.iteritems(), key=operator.itemgetter(1),
                    reverse=True):
      pattern = w[0]
      count = int(w[1])
