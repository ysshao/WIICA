...
def main():
  ...
  parser.add_argument('--source',
                      help='the name of source file, e.g. fft, md, etc.')
  parser.add_argument('--analysis_types',
                      choices=["opcode", "staticinst", "memory",
                               "branch", "basicblock", "register"],
                      help='Type of analysis. Separate multiple values with \
                            spaces. The supported analyses are shown.',
                      nargs="*")
  if len(sys.argv) < 2:
    os.system('python run_wiica.py -h')
    sys.exit(1)
